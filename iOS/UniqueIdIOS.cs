...
using NutraBiotics.iOS;
using Xamarin.Forms;

[assembly: Dependency(typeof(UniqueIdIOS))]
...
namespace NutraBiotics.iOS
{
	public class UniqueIdIOS : IDevice
	{
		...
		public string GetIdentifier()
		{
			...
			uint platformExpert = IOServiceGetMatchingService(
				0,
				IOServiceMatching("IOPlatformExpertDevice"));
			if (platformExpert != 0)
			{
				NSString key = (NSString)"IOPlatformSerialNumber";
				IntPtr serialNumber = IORegistryEntryCreateCFProperty(
					platformExpert,
					key.Handle,
					IntPtr.Zero,
					0);
